            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if hierarchy:
                    ...
                    if hierarchy['operational_manager_id']:
                        cur.execute("SELECT name FROM admins WHERE id = %s", (hierarchy['operational_manager_id'],))
                        om = cur.fetchone()
                        result['om_name'] = om['name'] if om else None
                    
                    if hierarchy['chief_manager_id']:
                        cur.execute("SELECT name FROM admins WHERE id = %s", (hierarchy['chief_manager_id'],))
                        um = cur.fetchone()
                ...
                if admin['role'] == 'manager':
                    ...
                    cur.execute("""
                        SELECT 
                            l.id, l.name, l.district, l.status,
                            s.end_date as subscription_end,
                            o.first_payment_date,
                            CASE 
                                WHEN s.end_date < NOW() + INTERVAL '1 day' THEN 'critical'
                                WHEN s.end_date < NOW() + INTERVAL '3 days' THEN 'warning'
                                ELSE 'ok'
                            END as urgency,
                        ...
                        LEFT JOIN subscriptions s ON l.id = s.listing_id AND s.end_date > NOW()
                        LEFT JOIN owners o ON l.owner_id = o.id
                        WHERE ml.manager_id = %s
                        ORDER BY 
                            CASE 
                                WHEN s.end_date < NOW() + INTERVAL '1 day' THEN 1
                                WHEN s.end_date < NOW() + INTERVAL '3 days' THEN 2
                                ELSE 3
                            END,
                            s.end_date ASC
                    """, (admin_id,))
                ...
                # Для УМ (Управляющего Менеджера)
                    ...
                    # Всего менеджеров
                    cur.execute("""
                        SELECT COUNT(DISTINCT mh2.manager_id) as total_managers
                        FROM manager_hierarchy mh
                        JOIN manager_hierarchy mh2 ON mh2.operational_manager_id = mh.operational_manager_id
                        WHERE mh.chief_manager_id = %s
                    """, (admin_id,))
